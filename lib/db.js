...
const stmtInsertProduct = db.prepare(`
  INSERT INTO products (id, name, category, price, colors, sizes, color, accent, description, featured, stock, sort, created_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
function createProduct(p) {
  ...
  stmtInsertProduct.run(p.id, p.name, p.category, p.price,
    ...
    p.featured ? 1 : 0, stock, p.sort || 0, new Date().toISOString());
...
module.exports = {
  ...
  countProducts, getAllProducts, getProductById, createProduct, updateProduct, deleteProduct, decrementStock, seedProductsIfEmpty
};
