...
ensureColumn('products', 'gender', "gender TEXT NOT NULL DEFAULT 'unisex'"); // sekcja: meska/damska/unisex
ensureColumn('products', 'name_en', 'name_en TEXT');   // tlumaczenia produktow (panel admina)
ensureColumn('products', 'name_de', 'name_de TEXT');
ensureColumn('products', 'desc_en', 'desc_en TEXT');
ensureColumn('products', 'desc_de', 'desc_de TEXT');

// Jednorazowo: ustaw sekcje wg nazwy (idempotentne — tylko gdy nadal domyslny 'unisex')
try {
  db.exec("UPDATE products SET gender='damska' WHERE gender='unisex' AND lower(name) LIKE '%damsk%'");
  db.exec("UPDATE products SET gender='meska' WHERE gender='unisex' AND (lower(name) LIKE '%męsk%' OR lower(name) LIKE '%mesk%')");
} catch (e) { console.error('Migracja gender wg nazwy:', e.message); }
...
ensureColumn('orders', 'parcel_locker_addr', 'parcel_locker_addr TEXT');
ensureColumn('orders', 'pay_token', 'pay_token TEXT');       // token do bezpiecznego sprawdzania statusu platnosci (P24)
ensureColumn('orders', 'p24_order_id', 'p24_order_id TEXT');  // numer transakcji nadany przez P24
...
const stmtCreateOrder = db.prepare(`
  INSERT INTO orders (id, user_id, customer_name, customer_email, customer_phone, customer_address, items, total, status, created_at, note, discount_code, discount, shipping, street, postal_code, city, delivery_method, parcel_locker, parcel_locker_addr, pay_token)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
function createOrder(o) {
  stmtCreateOrder.run(o.id, o.user_id ?? null, o.customer.name, o.customer.email,
    ...
    o.deliveryMethod || 'kurier', o.parcelLocker || '', o.parcelLockerAddr || '', o.payToken || '');
  return o;
}
...
function mapOrder(r) {
  return {
    ...
    payToken: r.pay_token || '', p24OrderId: r.p24_order_id || '',
    note: r.note || '', discountCode: r.discount_code || '', discount: r.discount || 0, shipping: r.shipping || 0,
    deliveryMethod: r.delivery_method || 'kurier', parcelLocker: r.parcel_locker || '', parcelLockerAddr: r.parcel_locker_addr || '',
    customer: {
...
function getOrder(id) { const r = stmtGetOrder.get(id); return r ? mapOrder(r) : null; }
...
const stmtSetP24OrderId = db.prepare('UPDATE orders SET p24_order_id = ? WHERE id = ?');
function setP24OrderId(id, p24OrderId) { return stmtSetP24OrderId.run(String(p24OrderId), id).changes > 0; }
...
function mapProduct(r) {
  ...
  return {
    ...
    featured: !!r.featured, stock: r.stock, sort: r.sort, gender: r.gender || 'unisex',
    name_en: r.name_en || '', name_de: r.name_de || '', desc_en: r.desc_en || '', desc_de: r.desc_de || '',
...
const stmtInsertProduct = db.prepare(`
  INSERT INTO products (id, name, category, price, colors, sizes, color, accent, description, featured, stock, sort, created_at, image, variants, gender, name_en, name_de, desc_en, desc_de)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
function createProduct(p) {
  ...
  stmtInsertProduct.run(p.id, p.name, p.category, p.price,
    ...
    p.variants ? JSON.stringify(p.variants) : null, p.gender || 'unisex',
    p.name_en || null, p.name_de || null, p.desc_en || null, p.desc_de || null);
...
const stmtUpdateProduct = db.prepare(`
  UPDATE products SET name = ?, category = ?, price = ?, colors = ?, sizes = ?,
    color = ?, accent = ?, description = ?, featured = ?, stock = ?, gender = ?,
    name_en = ?, name_de = ?, desc_en = ?, desc_de = ? WHERE id = ?
`);
function updateProduct(p) {
  return stmtUpdateProduct.run(p.name, p.category, p.price, JSON.stringify(p.colors),
    ...
    Number.isFinite(p.stock) ? p.stock : 0, p.gender || 'unisex',
    p.name_en || null, p.name_de || null, p.desc_en || null, p.desc_de || null, p.id).changes > 0;
}
...
module.exports = {
  ...
  createOrder, getOrder, countOrders, nextOrderId, getOrdersByUser, getAllOrders, updateOrderStatus, setP24OrderId, deleteOrder, decrementStock, setOrderNote,
